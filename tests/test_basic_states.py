...
async def test_basic_state_machine_with_simple_transitions():
    ...
    await instance.dispatch(ctx, hsm.Event(name='start'))
    assert instance.state() == '/BasicMachine/running'

    # Dispatch stop event
    await instance.dispatch(ctx, hsm.Event(name='stop'))
    assert instance.state() == '/BasicMachine/idle'
...
async def test_multiple_transitions_from_same_state():
    ...
    await instance.dispatch(ctx, hsm.Event(name='event1'))
    assert instance.state() == '/MultiTransitionMachine/state1'

    # Go back to idle
    await instance.dispatch(ctx, hsm.Event(name='back'))
    assert instance.state() == '/MultiTransitionMachine/idle'

    # Test second transition
    await instance.dispatch(ctx, hsm.Event(name='event2'))
...
async def test_self_transitions():
    ...
    await instance.dispatch(ctx, hsm.Event(name='increment'))
...
async def test_event_dispatching_during_lifecycle():
    ...
    await sm.dispatch(ctx, hsm.Event(name='next'))
...
async def test_unknown_events_ignored():
    ...
    await instance.dispatch(ctx, hsm.Event(name='unknown1'))
    await instance.dispatch(ctx, hsm.Event(name='unknown2'))
    await instance.dispatch(ctx, hsm.Event(name='unknown3', kind=hsm.EventKind))
...
async def test_event_object_dispatching():
    ...
    def proceed_effect(ctx: hsm.Context, inst: BasicInstance, event: hsm.Event) -> None:
    ...
    # Test Event object dispatching
    await instance.dispatch(ctx, hsm.Event(name='proceed'))
    ...
    assert instance.data.get('last_event_kind') == hsm.EventKind
    ...
    await instance.dispatch(ctx, hsm.Event(
        name='proceed',
        kind=hsm.EventKind
    ))
    assert instance.state() == '/EventTypeMachine/done'
